...
export async function GET(request: NextRequest) {
  try {
    ...
    // Fetch user debug info from backend
    const response = await fetch(`${API_BASE_URL}/admin/debug/user`, {
      method: 'GET',
      headers: {
    ...
    if (!response.ok) {
      ...
      return addSecurityHeaders(NextResponse.json(
        { 
          success: false, 
          message: errorData.message || 'Failed to fetch user debug info',
